...
export default function AdminLogin() {
  ...
  const handleLogin = async () => {
    ...
    setError("")
    try {
      const res = await axios.post("http://localhost:8080/admin/login", { username, password })
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 w-full max-w-sm space-y-6">
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-semibold tracking-widest text-gray-400 uppercase">Username</label>
            <input
              type="text"
              value={username}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold tracking-widest text-gray-400 uppercase">Password</label>
            <input
              type="password"
              value={password}
          ...
          <button
            ...
            style={{
              backgroundColor: loading || !username.trim() || !password.trim() ? "#e5e7eb" : "#1a2332",
              color: loading || !username.trim() || !password.trim() ? "#9ca3af" : "#ffffff",
